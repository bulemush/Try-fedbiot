...
def enable_adapter(model, package, adapter, **kwargs):
    ...
    if package == 'peft':
        if PEFT_IMPORT_ERROR is not None:
            raise ImportError(
                'Failed to import `peft`. Please install compatible '
                '`transformers` and `peft` versions before enabling LoRA '
                'or other PEFT adapters. A known working pair for this '
                'project is `transformers==4.29.2` with `peft==0.3.0`.'
...
class AdapterModel(nn.Module):
    ...
    def forward(self, disable_adapter=False, *args, **kwargs):
        if PeftModel is not None and isinstance(self.model,
                                                PeftModel) and disable_adapter:
    ...
    def generate(self, disable_adapter=False, *args, **kwargs):
        try:
            if PeftModel is not None and isinstance(
                    self.model, PeftModel) and disable_adapter:
                with self.model.disable_adapter():
    ...
    def save_model(self, path, state=0, merge_adapter=False):
        if merge_adapter and PeftModel is not None and isinstance(
