...
def get_generation_config(model_name, config):
    model_name = os.path.expanduser(model_name)
    generation_kwargs = {}
...
class FSChatBot(object):
    ...
    def use_raw_model(self):
        ...
        model_name, _ = parse_model_type(self.config.model.type)
        self.tokenizer, _ = get_tokenizer(model_name,
                                          get_model_cache_dir(self.config),
    ...
    def next_model(self):
        ...
        self.model = get_llm(self.config,
                             device_map='auto')
        self.generation_config = get_generation_config(model_name, self.config)
    ...
    def generate(self, input_text, generate_kwargs=None):
        ...
        input_ids = input_text_tokens.input_ids.to(self.device)
        attention_mask = input_text_tokens.attention_mask.to(self.device)
