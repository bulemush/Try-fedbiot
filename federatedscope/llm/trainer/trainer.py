    ...
    def _hook_on_fit_start_init(self, ctx):
        if ctx.cfg.llm.accelerator.use:
            ...
            if ctx.cur_mode in [MODE.TRAIN, MODE.FINETUNE]:
                ...
                if ctx.cfg[ctx.cur_mode].optimizer.type == 'AdamW':
                    optimizer_kwargs = dict(ctx.cfg[ctx.cur_mode].optimizer)
                    optimizer_kwargs.pop('type', None)
