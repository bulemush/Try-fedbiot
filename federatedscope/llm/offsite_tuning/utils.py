...
def build_cfg_for_alignment(config):
    ...
    # If alignment data is not explicitly configured, inherit the current
    # task dataset to avoid surprising extra data dependencies in offline runs.
    default_align_type = 'alpaca@llm'
    if new_cfg.llm.offsite_tuning.emu_align.data.type == default_align_type \
            and config.data.type != default_align_type:
        new_cfg.llm.offsite_tuning.emu_align.data.root = config.data.root
        new_cfg.llm.offsite_tuning.emu_align.data.type = config.data.type
        new_cfg.llm.offsite_tuning.emu_align.data.splits = config.data.splits
